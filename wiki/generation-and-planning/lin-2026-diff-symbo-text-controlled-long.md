...
source: lin-2026-diff-symbo-text-controlled-long.md
tags: ["symbolic-music","latent-diffusion","text-control","music-continuation","evaluation-validity"]
---

## Summary

Diff-Symbo generates eight-bar multitrack MIDI segments with text-conditioned latent diffusion, then extends them autoregressively using the previous segment's latent. The strongest controlled duration evidence is 32 bars. It reports improved attribute matching and listener ratings over selected baselines, while stronger classifier-free guidance reduces quality. [PDF pp. 3–7]

## Key Contributions

The main contribution is a generative architecture combining a learned text-attribute bottleneck, a multitrack VAE, and contextual diffusion. It belongs in generation-and-planning because its named attributes condition generation without verifying musical rules. The 19,345 templates are synthetic paraphrase patterns, not independent human captions. [PDF pp. 3–5]
...
![Diff-Symbo architecture and previous-segment cross-attention](assets/lin-2026-diff-symbo-text-controlled-long/fig-3-architecture.png)

Figure 3 (PDF p. 5) separates text conditioning from the previous-segment context introduced during fine-tuning.
...
![Eight-bar performance and guidance trade-off](assets/lin-2026-diff-symbo-text-controlled-long/tables-2-3-results.png)

Tables 2–3 (PDF p. 7) show why attribute accuracy alone cannot select the best generation setting.

### Limitations and artifact status
...
- [[overviews/symbolic-music-foundation-models]] — primary synthesis; segmentwise diffusion extends the architecture comparison.
- [[concepts/music-domain-inductive-biases]] — attribute conditioning is not a theory guarantee.
- [[generation-and-planning/wang-2025-notagen-advancing-musicality-in-symbolic]] — bar-aware ABC and length control.
- [[generation-and-planning/guo-2025-moonbeam-a-midi-foundation-model]] — event attributes and chord/metadata control.
